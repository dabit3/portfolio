...
/**
 * @ngdoc function
 * @name apifrontendApp.controller:BluePlateBrookhaven
 * @description
 * # EntreeController
 * Controller of the apifrontendApp
 */
angular.module('apifrontendApp')
  .controller('BluePlateBrookhaven', function ($scope, MenuItemsService, _, Notification, $routeParams, $location, $timeout) {
  	...
  	$timeout(function() {
	      ...
	      MenuItemsService.all()
	        .then(function (result) {
	          ...
	          var items = _.filter(result.data, function(r) {
	            return r.itemtype == 'blueplatebrookhaven';
	          });
	    ...
	    function addBlueplate() {
	      if(($scope.icon.value != undefined) && ($scope.price != undefined) && ($scope.description != undefined) && ($scope.name != undefined)) {
	        ...
	        item.itemtype = "blueplatebrookhaven";
