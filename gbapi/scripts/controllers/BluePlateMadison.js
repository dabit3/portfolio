...
/**
 * @ngdoc function
 * @name apifrontendApp.controller:BluePlateMadison
 * @description
 * # BluePlateMadison
 * Controller of the apifrontendApp
 */
angular.module('apifrontendApp')
  .controller('BluePlateMadison', function ($scope, MenuItemsService, _, Notification, $routeParams, $location, $timeout) {
  	...
  	$timeout(function() {
	      ...
	      MenuItemsService.all()
	        .then(function (result) {
	          ...
	          var items = _.filter(result.data, function(r) {
	            return r.itemtype == 'blueplatemadison';
	          });
	    ...
	    function addBlueplate() {
	      if(($scope.icon.value != undefined) && ($scope.price != undefined) && ($scope.description != undefined) && ($scope.name != undefined)) {
	        ...
	        item.itemtype = "blueplatemadison";
