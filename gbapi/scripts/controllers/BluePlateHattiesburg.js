...
/**
 * @ngdoc function
 * @name apifrontendApp.controller:BluePlateHattiesburg
 * @description
 * # BluePlateHattiesburg
 * Controller of the apifrontendApp
 */
angular.module('apifrontendApp')
  .controller('BluePlateHattiesburg', function ($scope, MenuItemsService, _, Notification, $routeParams, $location, $timeout) {
  	 ...
  	 $timeout(function() {
	      ...
	      MenuItemsService.all()
	        .then(function (result) {
	          ...
	          var items = _.filter(result.data, function(r) {
	            return r.itemtype == 'blueplatehattiesburg';
	          });
	    ...
	    function addBlueplate() {
	      if(($scope.icon.value != undefined) && ($scope.price != undefined) && ($scope.description != undefined) && ($scope.name != undefined)) {
	        ...
	        apptoadd.itemtype = "blueplatehattiesburg";
	        apptoadd.icon = $scope.icon.value;
	        apptoadd.price = $scope.price;
	        console.log("blueplate", apptoadd) 
